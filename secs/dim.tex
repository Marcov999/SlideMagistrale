...
\begin{frame}[t]
  ...
  \only<4->{
    ...
    \only<7-9>{caso 2: esiste (e quindi per ogni) $o \in X$ tale che
    ...
    \pause e, detto $K$ il compatto dato dalla visibilità per $\xi$ e $\eta$,}
    \only<10->{$\epsilon:=\displaystyle\liminf_{z \longrightarrow\eta}\inf_{y\in K}k_X(z,y)>0.$

    \setcounter{beamerpauses}{10}\pause Prendiamo $q_1,q_2\in K$ tali che
    $$G(q_1,q_2)<\inf_{x_1,x_2\in K}G(x_1,x_2)+\epsilon.$$\pause
